...
const CustomSlashCommands: React.FC<Props> = ({ guild, isAuth, error }: Props) => {
  ...
  async function deleteCommand(name: string) {
    try {
      const data = (await (
        await fetch(
          `${process.env["NEXT_PUBLIC_DASHBOARD_URL"]}/api/guilds/${guild?.id}/slash-commands?name=${encodeURIComponent(name)}`,
          {
            method: "DELETE",
          },
        )
        ...
        <AlertMessage
          message={
            <>
              <a
                href={
                  "https://discord.com/oauth2/authorize?client_id=889722439100141579&scope=applications.commands+bot&permissions=8"
                }
              >
                {t("re_authorize")}
              </a>
